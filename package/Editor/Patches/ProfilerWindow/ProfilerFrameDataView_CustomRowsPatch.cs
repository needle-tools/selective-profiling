		...
		private class Profiler_BuildRows_CollapseItems : EditorPatch
		{
			...
			private static bool ShouldCollapseRow(TreeView tree, TreeViewItem item, string name, out bool requestInfo)
			{
				...
				if (settings.CollapseNoImpactSamples && frameDataView != null && frameDataView.valid)
				{
					...
					{
						bool HasImpact(int id, int depth, float factor = 1)
						{
							...
							float k_MinMillis = 0.01f * factor;// / (depth*depth);
							float k_MinBytes = 1 * factor;
						...
						//
						if (ParentIsSelected(item) && HasImpact(item.id, item.depth, 0)) return false;
